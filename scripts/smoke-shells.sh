...
# How to run: bash ./scripts/smoke-shells.sh [path-to-pyenv]
...
PYENV_EXE_INPUT="${1:-${REPO_ROOT}/target/debug/pyenv}"
SMOKE_ROOT="${REPO_ROOT}/.tmp-shell-smoke-posix"
PYENV_ROOT_PATH="${SMOKE_ROOT}/.pyenv"
WORK_DIR="${SMOKE_ROOT}/work"

case "$PYENV_EXE_INPUT" in
  /*) PYENV_EXE="$PYENV_EXE_INPUT" ;;
  *) PYENV_EXE="${REPO_ROOT}/${PYENV_EXE_INPUT#./}" ;;
esac
...
cd "$WORK_DIR"
eval "$("$PYENV_EXE" init --no-rehash - bash)"
pyenv shell 3.13.12
