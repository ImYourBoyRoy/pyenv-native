...
#         crates/pyenv-gui/tauri.conf.json; optional EXPECTED_VERSION.
...
tauri_conf="$repo_root/crates/pyenv-gui/tauri.conf.json"
...
tauri_version=$(extract_version "$tauri_conf" 's/^[[:space:]]*"version":[[:space:]]*"\([^"]*\)".*/\1/p' 'tauri.conf.json version')

failed=0

if [ "$cargo_version" != "$py_version" ] || [ "$cargo_version" != "$init_version" ] || [ "$cargo_version" != "$tauri_version" ]; then
  ...
  printf 'Fix with: pwsh -NoLogo -NoProfile -File ./scripts/set-version.ps1 -Version <semver>\n' >&2
  failed=1
fi
