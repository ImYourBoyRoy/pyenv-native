...
pub(super) const SYSTEM_COMMAND_DOCS: &[CommandDoc] = &[
    ...
    CommandDoc {
        ...
        help: &[
            ...
            "  pyenv prefix 3.14.7        Show prefix for a specific version",
        ],
        completions: &[],
    },
    CommandDoc {
    ...
    CommandDoc {
        ...
        help: &[
            ...
            "  - On macOS, verifies Xcode Command Line Tools and OpenSSL TLS headers",
            "  - On Termux/Android, verifies clang/make and TLS/library headers",
            "",
            "Use `--json` for structured output suitable for scripts and automation.",
            "Use `--fix` to apply safe automated repairs like refreshing shims, creating",
            "the managed directory layout, installing Termux packages, and best-effort",
            "macOS CLT/OpenSSL setup. Use `-f|--force` with `--fix` to skip confirmation.",
            "",
            "Examples:",
    ...
    CommandDoc {
        name: "preflight",
        summary: "Show platform intelligence and install-readiness before building Python",
        usage: "Usage: pyenv preflight [--json]",
        help: &[
            "Builds a tool-relevant snapshot of the host OS, architecture, install strategy,",
            "and source-build prerequisites (Xcode/CLT + OpenSSL on macOS, Termux packages on",
            "Android, compiler toolchain on Linux). Intended for humans and agents before",
            "`pyenv install`.",
            "",
            "Exit code 0 means installs should proceed; non-zero means blocking prerequisites.",
            "",
            "Examples:",
            "  pyenv preflight",
            "  pyenv preflight --json",
            "  pyenv environment --json   # alias",
        ],
        completions: &["--json"],
    },
    CommandDoc {
        name: "environment",
        summary: "Alias for `preflight` (OS/toolchain facts for agents and users)",
        usage: "Usage: pyenv environment [--json]",
        help: &[
            "Same output as `pyenv preflight`. Prefer `preflight` in docs; `environment` exists",
            "for agent-friendly naming.",
        ],
        completions: &["--json"],
    },
    CommandDoc {
    ...
    CommandDoc {
        ...
        help: &[
            ...
            "",
            "Companion base-venv keys default to false. Set them true only when you want",
            "a protected base venv created and preferred after each install.",
        ],
