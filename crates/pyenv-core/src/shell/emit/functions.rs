...
fn render_pwsh_shell_function(exe_path: &str) -> Vec<String> {
    vec![
        ...
        "}".to_string(),
        "function Invoke-PyenvExpression {".to_string(),
        "  param([string[]]$ShellCmds)".to_string(),
        "  if ($null -eq $ShellCmds -or $ShellCmds.Count -eq 0) { return }".to_string(),
        "  $expr = (($ShellCmds | Where-Object { $null -ne $_ }) -join \"`n\").Trim()".to_string(),
        "  if ($expr.Length -gt 0) { Invoke-Expression $expr }".to_string(),
        "}".to_string(),
        ...
        "      if ($LASTEXITCODE -eq 0) { Invoke-PyenvExpression $shellCmds }".to_string(),
        "    }".to_string(),
        "    'activate' {".to_string(),
        ...
        "      if ($LASTEXITCODE -eq 0) { Invoke-PyenvExpression $shellCmds }".to_string(),
        "    }".to_string(),
        // init / virtualenv-init must return stdout on the PowerShell success stream.
        // Invoke-PyenvPassthrough inherits the console, so `(pyenv init - pwsh)` captures
        // nothing and `iex` fails with "empty string" once this function shadows pyenv.exe
        // (duplicate profile lines, nested inits, or re-sourcing $PROFILE).
        "    { $_ -in @('init', 'virtualenv-init') } {".to_string(),
        "      $output = Invoke-PyenvCaptured $pyenvExe (@([string]$command) + @($arguments))".to_string(),
        "      if ($output.Count -gt 0) { $output }".to_string(),
        "    }".to_string(),
        "    default {".to_string(),
