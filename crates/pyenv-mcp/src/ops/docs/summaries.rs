    ...
    vec![
        ...
        ToolSummary {
            ...
            arguments: vec![
                ...
                ToolArgument { name: "install_root".to_string(), required: false, data_type: "path".to_string(), description: "Override the install root that should appear in example commands.".to_string() },
                ToolArgument { name: "server_name".to_string(), required: false, data_type: "string".to_string(), description: "Override the MCP server name used in the returned client config snippet.".to_string() },
                ToolArgument { name: "mcp_command".to_string(), required: false, data_type: "string".to_string(), description: "Override the MCP executable path shown in the client config snippet.".to_string() },
                ToolArgument { name: "pyenv_root".to_string(), required: false, data_type: "path".to_string(), description: "Override PYENV_ROOT in the generated client config.".to_string() },
            ],
            example_input: Some(json!({ "install_root": "~/.pyenv" })),
        },
        ToolSummary {
            tool_name: "inspect_environment".to_string(),
            use_when: "You want a status snapshot of selected versions, origins, and the active managed venv.".to_string(),
            returns: "The same structured status payload available from pyenv status --json.".to_string(),
            side_effects: "Read-only.".to_string(),
        ...
        ToolSummary {
            ...
            arguments: vec![
                ToolArgument { name: "version".to_string(), required: true, data_type: "string".to_string(), description: "Requested runtime version or prefix, such as 3.12, 3.14.7, or pypy3.11.".to_string() },
        ...
        ToolSummary {
            ...
            example_input: Some(json!({ "versions": ["3.14"] })),
        },
        ToolSummary {
        ...
        ToolSummary {
            ...
            arguments: vec![
                ...
                ToolArgument { name: "force".to_string(), required: false, data_type: "boolean".to_string(), description: "When omitted or true, apply safe automated repairs. When false, return the planned fixes without applying them.".to_string() },
            ],
        ...
        ToolSummary {
            ...
            side_effects: "Read-only.".to_string(),
            arguments: vec![
                ToolArgument { name: "target".to_string(), required: true, data_type: "string".to_string(), description: "Target environment version (e.g. 3.14.7) or managed venv spec (e.g. 3.14.7/envs/api).".to_string() },
        ...
        ToolSummary {
            ...
            use_when: "You want to check which packages in a target environment have newer versions available on PyPI before calling pip_update.".to_string(),
            returns: "JSON list of {name, version, latest_version}. Copy names or name==latest_version pins into pip_update; this blob is not a pip_update argument.".to_string(),
            side_effects: "Read-only.".to_string(),
            arguments: vec![
        ...
        ToolSummary {
            ...
            use_when: "You want to statically analyze a requirements.txt file or remote HTTPS URL against the installed packages before installation to identify conflicts.".to_string(),
            ...
            arguments: vec![
                ToolArgument { name: "target".to_string(), required: true, data_type: "string".to_string(), description: "Target environment version or managed venv spec.".to_string() },
                ToolArgument { name: "path_or_url".to_string(), required: true, data_type: "string".to_string(), description: "Local path to a requirements.txt or a remote HTTPS URL. http:// is rejected.".to_string() },
                ToolArgument { name: "project_dir".to_string(), required: false, data_type: "path".to_string(), description: "Optional project directory.".to_string() },
            ],
        ...
        ToolSummary {
            ...
            use_when: "You want to install dependencies from a requirements.txt file or HTTPS URL. Run pip_precheck first to identify conflicts safely.".to_string(),
        ...
        ToolSummary {
            ...
            use_when: "You want to upgrade named packages, optional name==version pins, or every outdated package after pip_outdated.".to_string(),
            returns: "Structured outcome with stdout/stderr lines and exit code. Updates pip itself first when pip is outdated. all=true fails if the outdated scan cannot run.".to_string(),
            side_effects: "Modifies and upgrades library files inside the target environment.".to_string(),
            arguments: vec![
                ToolArgument { name: "target".to_string(), required: true, data_type: "string".to_string(), description: "Target environment version or managed venv spec.".to_string() },
                ToolArgument { name: "packages".to_string(), required: false, data_type: "array<string>".to_string(), description: "Package names or name==version pins. Required when all is false.".to_string() },
                ToolArgument { name: "all".to_string(), required: false, data_type: "boolean".to_string(), description: "Upgrade every outdated package. Fails closed if pip_outdated cannot run.".to_string() },
                ToolArgument { name: "project_dir".to_string(), required: false, data_type: "path".to_string(), description: "Optional project directory.".to_string() },
            ],
            example_input: Some(json!({ "target": "3.14.7/envs/api", "packages": ["certifi", "cryptography"] })),
        },
        ToolSummary {
            tool_name: "get_config".to_string(),
            use_when: "You need to read pyenv-native settings such as install.bootstrap_pip or windows.registry_mode.".to_string(),
            returns: "Either the full config TOML (stdout lines) or a single key value.".to_string(),
            side_effects: "Read-only.".to_string(),
            arguments: vec![
                ToolArgument { name: "key".to_string(), required: false, data_type: "string".to_string(), description: "Optional dotted config key. Omit to show the full config.".to_string() },
            ],
            example_input: Some(json!({ "key": "windows.registry_mode" })),
        },
        ToolSummary {
            tool_name: "set_config".to_string(),
            use_when: "You need to persist a pyenv-native config value after confirming with the user.".to_string(),
            returns: "Empty success payload when the key is saved.".to_string(),
            side_effects: "Writes PYENV_ROOT/config.toml.".to_string(),
            arguments: vec![
                ToolArgument { name: "key".to_string(), required: true, data_type: "string".to_string(), description: "Dotted config key, such as install.bootstrap_pip.".to_string() },
                ToolArgument { name: "value".to_string(), required: true, data_type: "string".to_string(), description: "Value to persist (true/false, pep514/disabled, paths, etc.).".to_string() },
            ],
            example_input: Some(json!({ "key": "plugins.search_path", "value": "true" })),
        },
        ToolSummary {
            tool_name: "self_update".to_string(),
            use_when: "You need to check whether pyenv-native itself has a newer GitHub Release, or apply an update after the user confirms.".to_string(),
            returns: "Check or update status lines from the native self-updater.".to_string(),
            side_effects: "check=true is read-only. Applying an update requires yes=true and replaces the installed binaries.".to_string(),
            arguments: vec![
                ToolArgument { name: "check".to_string(), required: false, data_type: "boolean".to_string(), description: "When true, compare versions without installing.".to_string() },
                ToolArgument { name: "yes".to_string(), required: false, data_type: "boolean".to_string(), description: "Required to apply an update. Defaults to false.".to_string() },
                ToolArgument { name: "force".to_string(), required: false, data_type: "boolean".to_string(), description: "Re-run the installer even when already on the target tag.".to_string() },
                ToolArgument { name: "github_repo".to_string(), required: false, data_type: "string".to_string(), description: "Override owner/repo. Defaults to imyourboyroy/pyenv-native.".to_string() },
                ToolArgument { name: "tag".to_string(), required: false, data_type: "string".to_string(), description: "Optional release tag to install instead of latest.".to_string() },
            ],
            example_input: Some(json!({ "check": true })),
        },
        ToolSummary {
            tool_name: "venv_upgrade".to_string(),
            use_when: "A managed venv should move to a newer already-installed Python runtime while keeping its pip packages.".to_string(),
            returns: "Upgrade progress lines and the new env spec. Fails closed if the source package inventory cannot run or if package restore fails, leaving the source venv unchanged.".to_string(),
            side_effects: "Creates a temporary managed venv, restores packages, removes the old env, then renames to the original name.".to_string(),
            arguments: vec![
                ToolArgument { name: "spec".to_string(), required: true, data_type: "string".to_string(), description: "Managed env spec such as 3.14.7/envs/api or a unique env name.".to_string() },
                ToolArgument { name: "new_runtime".to_string(), required: true, data_type: "string".to_string(), description: "Installed runtime version or prefix to rebuild on.".to_string() },
                ToolArgument { name: "force".to_string(), required: false, data_type: "boolean".to_string(), description: "Replace the destination env if it already exists.".to_string() },
                ToolArgument { name: "set_local".to_string(), required: false, data_type: "boolean".to_string(), description: "Write the new env spec into the current .python-version file.".to_string() },
                ToolArgument { name: "project_dir".to_string(), required: false, data_type: "path".to_string(), description: "Optional project directory for context.".to_string() },
            ],
            example_input: Some(json!({ "spec": "3.14.7/envs/api", "new_runtime": "3.14" })),
        },
    ]
}
